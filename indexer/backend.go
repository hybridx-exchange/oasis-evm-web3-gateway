...
type BackendFactory func(runtimeID common.Namespace, storage storage.Storage, sb filters.SubscribeBackend) (Backend, error)
...
func (ib *indexBackend) Index(oasisBlock *block.Block, txResults []*client.TransactionWithResults) error {
	...
	if err := ib.storage.Store(blockRef); err != nil {
		return err
	}
...
func (ib *indexBackend) storeIndexedRound(round uint64) error {
	ib.indexedRoundMutex.Lock()
	defer ib.indexedRoundMutex.Unlock()
	...
	return ib.storage.Update(r)
}

// QueryLastIndexedRound returns the last indexed round.
func (ib *indexBackend) QueryLastIndexedRound() uint64 {
	ib.indexedRoundMutex.Lock()
	indexedRound, err := ib.storage.GetContinuesIndexedRound()
	if err != nil {
		ib.indexedRoundMutex.Unlock()
		return 0
	}
	ib.indexedRoundMutex.Unlock()
...
func (ib *indexBackend) GetLogs(blockHash ethcommon.Hash, startRound, endRound uint64) ([]*model.Log, error) {
	return ib.storage.GetLogs(blockHash.String(), startRound, endRound)
}

// Close closes postgresql backend.
func (ib *indexBackend) Close() {
	ib.logger.Info("Psql backend closed!")
}

// newIndexBackend creates a Backend.
func newIndexBackend(runtimeID common.Namespace, storage storage.Storage, sb filters.SubscribeBackend) (Backend, error) {
	b := &indexBackend{
		runtimeID:         runtimeID,
